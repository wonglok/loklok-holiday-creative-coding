...
export class PhysicsInfluences {
  ...
  constructor({ array, mini }) {
    ...
    let headerCode = /* glsl */ `
        ...
        void customCompute (float index, inout vec3 position, inout vec3 velocity) {
          float uv_Y = index / ${influencerCount.toFixed(1)};
          ...
          velocity.y = -500.0;
          vec3 spinner = 1.5 * vec3(rotationY(1.0) * vec4(vec3(position.x, 0.0, position.z) * 1.0, 1.0));

          velocity.xz += spinner.xz;
        ...
        void computeAllInfluence (inout vec3 position, inout vec3 velocity) {
          for (int ii = 0; ii < ${influencerCount.toFixed(0)}; ii++) {
            ...
            customCompute(index, position, velocity);
          }
